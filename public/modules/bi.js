...
function renderWorkspaceLayout() {
  formContainer.innerHTML = `
    <div class="space-y-3">
      <div>
        <h3 class="text-indigo-600 text-xs font-black uppercase tracking-wider">📊 Analytics Core</h3>
        <p class="text-[10px] text-stone-400 leading-snug">Continuous mathematical tracking engine providing high-level operational breakdowns.</p>
      </div>
  ...
  viewContainer.innerHTML = `
    <div class="space-y-4 text-stone-900">
      <div class="grid grid-cols-2 gap-2 text-xs">
        <div class="bg-white p-2.5 rounded-xl border"><span class="text-[9px] uppercase font-bold text-stone-400 block">Ops Load</span><span id="bi-ops-load" class="text-lg font-black block mt-0.5">0</span></div>
        <div class="bg-white p-2.5 rounded-xl border"><span class="text-[9px] uppercase font-bold text-stone-400 block">Dispute Value</span><span id="bi-fin-risk" class="text-lg font-black text-rose-600 block mt-0.5">$0</span></div>
        <div class="bg-white p-2.5 rounded-xl border"><span class="text-[9px] uppercase font-bold text-stone-400 block">CRM Pipeline</span><span id="bi-sales-pipe" class="text-lg font-black text-indigo-600 block mt-0.5">$0</span></div>
        <div class="bg-white p-2.5 rounded-xl border"><span class="text-[9px] uppercase font-bold text-stone-400 block">VIP Density</span><span id="bi-vip-depth" class="text-lg font-black text-amber-500 block mt-0.5">0%</span></div>
      </div>
...
export async function refresh() {
  try {
    const res = await secureFetch('/api/bi/analytics'); 
    if (!res.ok) return;
    ...
    document.getElementById('bi-ops-mix').innerHTML = (ops.breakdown || []).map(c => `<div class="flex justify-between"><span>${c._id || 'Unassigned'}</span><span class="text-amber-400 font-bold">${c.count || 0} tasks</span></div>`).join('') || '<span class="italic text-stone-600">No operations active</span>';
  } catch(e){
